...
class AdminController extends Controller
{
    ...
    {
        $datas = User::where('role', 'customer')->get();
        $dataArray[] = array("Id", "Name", "Phone", "Email", "Status", "Delete Status", "Created At", "Updated At");
    ...
    //     $user = User::where('deleteId', 0)->when($formid, function ($model) use ($formid) {
    ...
    public function indexEnquiry()
    {
        ...
        return view('admin.master.enquiry', compact('enquiries'));
    }

    public function updateEnquiry(Request $request)
    {
        $model = Enquiries::find($request->hiddenId);
        $model->status = $request->status;
        $model->update();
        Session()->flash('alert-success', "Status Updated Succesfully");
        return redirect()->back();
    }
